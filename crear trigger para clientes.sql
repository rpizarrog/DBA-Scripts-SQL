...
trigger trclientes
  ...
  begin
    if inserting then
      dbms_output.put_line('Se agregó un cliente, pero cual ? '  || :new.idcliente);
        
      select sqprueba.nextval, sysdate, to_char(sysdate, 'HH24:MI:SS'),
     ...
     if deleting then
      dbms_output.put_line('Se eliminó, pero cual ? '  || :old.idcliente);
      select sqprueba.nextval, sysdate, to_char(sysdate, 'HH24:MI:SS'),
     ...
     if updating then
      dbms_output.put_line('Se actualizó, pero cual ? '  || :old.idcliente);
select sqprueba.nextval, sysdate, to_char(sysdate, 'HH24:MI:SS'),
